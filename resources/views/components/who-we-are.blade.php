                    ...
                    <div class="bg-gradient-to-r from-slate-800/60 to-slate-900/60 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-white/10 shadow-2xl">
                        ...
                        <div class="grid lg:grid-cols-3 gap-8 items-center">
                            ...
                            <div class="lg:col-span-2">
                                <div class="flex items-start gap-4 mb-6 max-sm:flex-col">
                                    ...
                                    <div class="max-sm:text-center">
                                        ...
                                        @if(isset($ctaData['highlight']))
                                            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium" style="background: rgba(30, 192, 141, 0.2); color: var(--voip-link);">
                                                <i class="uil uil-star text-sm"></i>
                                                {{ $ctaData['highlight'] }}
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <!-- Contact Methods -->
                                @if(isset($ctaData['contact_info']) && is_array($ctaData['contact_info']))
                                    <div class="grid md:grid-cols-3 gap-4">
                                        @foreach($ctaData['contact_info'] as $contact)
                                            <a href="{{ $contact['action'] ?? '#' }}" class="flex items-center gap-3 p-4 rounded-xl border border-white/10 transition-all duration-300 group hover:scale-105" onmouseover="this.style.borderColor='var(--voip-link)'; this.style.boxShadow='0 0 20px rgba(30, 192, 141, 0.3)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.1)'; this.style.boxShadow='none'">
                                                <div class="flex-shrink-0">
                                                    <i class="{{ $contact['icon'] ?? 'uil uil-phone' }} text-lg group-hover:scale-110 transition-transform" style="color: var(--voip-link);"></i>
                                                </div>
                                                <div class="min-w-0 flex-1">
                                                    <p class="text-sm text-slate-400 mb-1">{{ $contact['label'] }}</p>
                                                    <p class="text-white font-medium text-sm truncate">{{ $contact['value'] }}</p>
                                                </div>
                                            </a>
                                        @endforeach
                                    </div>
                            ...
                            <!-- Right: Stats -->
                            @if(isset($ctaData['stats']) && is_array($ctaData['stats']))
                                <div class="lg:col-span-1">
                                    <div class="grid grid-cols-3 lg:grid-cols-1 gap-6">
                                        @foreach($ctaData['stats'] as $stat)
                                            <div class="text-center lg:text-left">
                                                <div class="text-2xl lg:text-3xl font-bold mb-1" style="color: var(--voip-link);">{{ $stat['number'] }}</div>
