...
@section('title', 'Sawtic | AI Call Center Solutions for Banks & Financial Institutions in UAE')

@push('styles')
<link rel="stylesheet" href="{{ asset('assets/css/solutions/finance-banking.css') }}">
@endpush

@section('content')

@php
$industryService = new \App\Services\IndustryDataService('finance-banking');
@endphp
...
{{-- CTA Conversion Section - "Ready to Transform Banking Operations?" --}}
...
@push('scripts')
<script>
// Finance Banking Solutions Page Analytics
document.addEventListener('DOMContentLoaded', function() {
    ...
    audioDemos.forEach(audio => {
        audio.addEventListener('play', function() {
            console.log('Banking voice demo played:', this.dataset.demoType);
        });
    });
    ...
    if (roiCalculator) {
        roiCalculator.addEventListener('change', function() {
            console.log('Banking ROI calculator interaction');
        });
    }
    ...
    ctaButtons.forEach(button => {
        button.addEventListener('click', function() {
            console.log('Banking CTA clicked:', this.dataset.ctaTrack);
        });
    });
});
</script>
